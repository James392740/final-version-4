...
int OthelloBoard::miniMax(int cur_depth,int depth,int alpha,int beta,bool maxplayer)
{
    if(cur_depth==depth || cur_depth==0)
    {
        ...
        for(int i=0;i<next_valid_spots.size();i++)
        {
            ...
            //middle
            else if(2<=next_valid_spots[i].x<=5 && 2<=next_valid_spots[i].y<=5)
            {
                mid_num++;
            }
        }
        ...
        if(cur_player==BLACK)
        {
            total+=corner_num*800-bad_X_num*300-bad_C_num*300+mobility_num*30+good_C_num*250+good_X_num*200+side_num*100+more_num*30+mid_num*20;
        }
